# These operators assemble the scalar Poisson weak form for the manufactured
...
# `a(v, u) = ∫Ω ∇v · ∇u dΩ`. The local kernel fills only one triangle of the
# symmetric element matrix and mirrors the entries explicitly.
struct NewtonFractalPoissonDiffusion{F}
  field::F
end

function cell_matrix!(local_matrix, operator::NewtonFractalPoissonDiffusion, values::CellValues)
