...
# descriptor and a cell-measure policy, and the rest of the library treats that
# wrapper as an ordinary domain whose active leaves are the physical leaves
# only. For the current library scope, the region backend is an implicit
# classifier with finite-cell quadrature support.
#
...
#   including cached inside/cut/outside classification and physical cut-cell
#   quadratures,
# - the domain's cell-measure policy decides whether assembly uses those true
#   physical quadratures directly or a stabilized finite-cell measure.
#
...
Abstract supertype for policies that describe which cell-volume measure a
[`PhysicalDomain`](@ref) uses during assembly.

The policy is deliberately separate from [`AbstractPhysicalRegion`](@ref): the
region describes geometry and physical classification, while the cell measure
describes how cut-cell volume integrals are weighted in algebraic operators.
"""
abstract type AbstractCellMeasure end

"""
    PhysicalMeasure()

Use the true physical cell measure for cut-cell integration.

For a [`PhysicalDomain`](@ref), this means that automatic cell quadratures on
cut leaves integrate over `Ω_phys` only. This is the default and preserves the
standard finite-cell behavior used by earlier versions of the package.
"""
struct PhysicalMeasure <: AbstractCellMeasure end

"""
    FiniteCellExtension(alpha)

Use the stabilized finite-cell measure `(1 - α)Ω_phys + αΩ_full`.

`alpha` must be finite and lie in `[0, 1]`. The true physical part keeps
effective weight one because `Ω_full` already contains `Ω_phys`; the fictitious
part therefore receives weight `α`. This policy is useful for immersed-domain
problems where tiny cut cells would otherwise lead to singular or badly
conditioned local mass and stiffness blocks.
"""
struct FiniteCellExtension{T<:AbstractFloat} <: AbstractCellMeasure
  alpha::T

  function FiniteCellExtension{T}(alpha::T) where {T<:AbstractFloat}
    isfinite(alpha) || throw(ArgumentError("finite-cell extension alpha must be finite"))
    zero(T) <= alpha <= one(T) ||
      throw(ArgumentError("finite-cell extension alpha must lie in [0, 1]"))
    return new{T}(alpha)
  end
end

function FiniteCellExtension(alpha::Real)
  checked = float(alpha)
  return FiniteCellExtension{typeof(checked)}(checked)
end

_checked_cell_measure(measure::AbstractCellMeasure) = measure
...
description and a cut-cell measure policy.
...
background leaves are dropped automatically. For cut leaves, the true physical
quadrature is generated by the region, while `cell_measure` decides which
volume measure is used automatically during assembly and state transfer.
`PhysicalMeasure()` integrates over `Ω_phys`; [`FiniteCellExtension`](@ref)
integrates over `(1 - α)Ω_phys + αΩ_full`.

Leaves whose physical intersection has zero volume, such as a cell that only
touches the implicit boundary at one corner, are treated as outside and are
therefore trimmed as well.
...
function PhysicalDomain(background::Domain{D,T}, region::AbstractPhysicalRegion;
                        cell_measure=PhysicalMeasure()) where {D,T<:AbstractFloat}
  checked_measure = _checked_cell_measure(cell_measure)
  return PhysicalDomain{D,T,typeof(background),typeof(region),typeof(checked_measure)}(background,
                                                                                       region,
                                                                                       checked_measure)
end
...
@inline _cell_measure(::Domain) = PhysicalMeasure()
@inline _cell_measure(domain::PhysicalDomain) = domain.cell_measure
...
@inline _physical_cell_quadrature(::Domain, leaf::Integer, quadrature_shape) = nothing

function _physical_cell_quadrature(domain::PhysicalDomain{D,T}, leaf::Integer,
                                   quadrature_shape::NTuple{D,<:Integer}) where {D,T<:AbstractFloat}
...
@inline _assembly_cell_quadrature(::Domain, leaf::Integer, quadrature_shape) = nothing
...
function _assembly_cell_quadrature(domain::PhysicalDomain{D,T}, physical,
  ...
  checked_shape = ntuple(axis -> _checked_positive(quadrature_shape[axis],
                                                   "quadrature_shape[$axis]"), D)
  full = TensorQuadrature(T, checked_shape)
  physical_weight = one(T) - alpha
  points = NTuple{D,T}[]
  weights = T[]
  sizehint!(points, point_count(physical) + point_count(full))
  sizehint!(weights, point_count(physical) + point_count(full))

  for point_index in 1:point_count(physical)
    push!(points, point(physical, point_index))
    push!(weights, physical_weight * weight(physical, point_index))
  end

  for point_index in 1:point_count(full)
    push!(points, point(full, point_index))
    push!(weights, alpha * weight(full, point_index))
  end
